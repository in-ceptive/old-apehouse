...
import nftContract from "../ABI/ApeHouseCollection.json";
import {
  nftContractAddress,
  networkDeployedTo,
    ...
    if (wallet.network === networksMap[networkDeployedTo]) {
      try {
        ...
        const metadataCid = await saveContent(
          new File([data], `Apehouse-${tokenId}.json`)
        );
        const metadataIpfsHash = `ipfs://${metadataCid}/Apehouse-${tokenId}.json`;
    ...
    <div className="create section__padding">
      {wallet.registred ? (
        ...
        <div className="listing-text">
          <p>You must be registered to mint Apehouse tokens</p>
          <div className="mint-btn">
            <button>
