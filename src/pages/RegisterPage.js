    ...
    if (wallet.network === networksMap[networkDeployedTo]) {
      try {
        ...
        const metadataCid = await saveContent(
          new File([data], `Apehouse-Artist-${Number(tokenId)}.json`)
        );
        const metadataIpfsHash = `ipfs://${metadataCid}/Apehouse-Artist-${Number(
          tokenId
        )}.json`;
  ...
  const edit = async () => {
    if (
      ...
      try {
        ...
        const metadataCid = await saveContent(
          new File([data], `Apehouse-Artist-${profile.tokenId}.json`)
        );
        const metadataIpfsHash = `ipfs://${metadataCid}/Apehouse-Artist-${profile.tokenId}.json`;
